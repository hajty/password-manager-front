...
export class PasswordsService {
    ...
    getPasswords(token: Token): Observable<IPassword[]> {
        return this.http
            .get<IPassword[]>(URL_PASSWORDS, {
                headers: {
                    authorization: token.accessToken,
                },
                observe: 'body',
            })
            .pipe(catchError(this.handleError<IPassword[]>('getPasswords', [])));
    }
    ...
    postPassword(token: Token, password: IPassword): Observable<IPassword[] | HttpResponse<any>> {
        return this.http
            ...
            .pipe(catchError(this.handleError<IPassword[]>('postPassword', [])));
    }
    ...
    updatePassword(token: Token, password: IPassword): Observable<IPassword[] | HttpResponse<any>> {
        ...
        return this.http
            ...
            .pipe(catchError(this.handleError<IPassword[]>('updatePassword', [])));
    }

    deletePassword(token: Token, password: IPassword): Observable<IPassword[] | HttpResponse<any>> {
        return this.http
            .delete(`${URL_PASSWORD}/${password._id}`, {
                headers: {
                    authorization: token.accessToken,
                },
                observe: 'response',
            })
            .pipe(catchError(this.handleError<IPassword[]>('deletePassword', [])));
    }
}
